...
export function saveNav(ids: string[], index: number, backUrl: string, backQuery?: string): void {
  try {
    sessionStorage.setItem(CADASTRE_NAV_KEY, JSON.stringify({ ids, index, backUrl, backQuery } satisfies CadastreNavContext));
  } catch {
